...
class ESEWCPlugin(SupervisedPlugin):
    """
    ...
    def before_backward(
        ...
        for exp, importances in self.importances.items():
            diff_weights = current_flatten_params - self.prev_params[exp]
            penalty += (diff_weights * importances).norm(2) ** 2
    ...
    def after_training_exp(self, strategy: SupervisedTemplate, *args, **kwargs):
        ...
        self.importances[strategy.clock.train_exp_counter] = importances
        self.saved_params[strategy.clock.train_exp_counter] = deepcopy(
            strategy.model.state_dict()
        )
        self.prev_params[strategy.clock.train_exp_counter] = flat_params(strategy.model)
...
class ESEWC(SupervisedTemplate):
    ...
    def __init__(
        ...
        **base_kwargs,
        ...
        cewc = ESEWCPlugin(ewc_lambda, threshold)
    ...
    def get_all_stored_weights(self):
        for plugin in self.plugins:
            if isinstance(plugin, ESEWCPlugin):
